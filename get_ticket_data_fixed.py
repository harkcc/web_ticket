    ...
    def process(self):
        ...
        try:
            ...
            for _, row in df.iloc[2:last_product_row + 1].iterrows():
                if pd.isna(row.iloc[0]):  # 如果序号为空，跳过
                    continue

                row_count += 1
                msku = row.iloc[1]  # 第二列是MSKU
                fnsku = row.iloc[2]  # 第三列是FNSKU
                product_name = row.iloc[3]  # 第四列是品名
                sku = row.iloc[4]  # 第五列是SKU
                total_quantity = row.iloc[5]  # 第六列是发货总数量
                ...
                # 处理每箱数量（使用动态检测的起始列）
                max_col_idx = len(row) - 1
                for col_idx, col in enumerate(df.columns[box_start_col:], start=box_start_col):
                    # 检查列索引是否在有效范围内
                    if col_idx > max_col_idx:
                        print(f"  Warning: Column index {col_idx} exceeds row length {len(row)}, skipping")
                        break
                    
                    # 使用iloc进行位置访问，避免FutureWarning
                    try:
                        quantity = row.iloc[col_idx]
                    except IndexError:
                        print(f"  Warning: Cannot access column index {col_idx}, skipping")
                        continue
...
# class SimplePackingListProcessor:
    ...
    def process(self, template_name=None):
        ...
        try:
            ...
            # 获取Shipment ID（删除列后的第1行第1列）
            try:
                self.shipment_id = str(df.iloc[1, 1])  # 第2行第2列（鹏城罗马尼亚空派）
                if pd.isna(self.shipment_id):
                    raise ValueError("Shipment ID is empty")
            except Exception as e:
                raise ValueError(f"Invalid Shipment ID: {str(e)}")
